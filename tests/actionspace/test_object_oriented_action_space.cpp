...
TEST_F(SimpleOOActionFixture, ActionGenerationTest) {
    ...
    EXPECT_NEAR(124.969, action->start_pos(0), 0.001);
    EXPECT_NEAR(145.442, action->start_pos(1), 0.001);
    EXPECT_EQ(2, action->theta);
    EXPECT_EQ(2.5, action->speed);
    EXPECT_EQ(5, action->duration);

    action = m_actionspace.get_action(10);
    EXPECT_NEAR(45.4422, action->start_pos(0), 0.0001);
    EXPECT_NEAR(175.031, action->start_pos(1), 0.001);
    ...
    EXPECT_NEAR(75.0312, action->start_pos(0), 0.0001);
    EXPECT_NEAR(254.558, action->start_pos(1), 0.001);
    EXPECT_NEAR(-1.14159, action->theta, 0.00001);
    EXPECT_EQ(0, action->speed);
    EXPECT_EQ(5, action->duration);

    action = m_actionspace.get_action(30);
    EXPECT_NEAR(154.558, action->start_pos(0), 0.001);
    EXPECT_NEAR(224.969, action->start_pos(1), 0.001);
...
TEST_F(ComplexOOActionFixture, ActionGenerationTest) {
    ...
    EXPECT_NEAR(88.5969, action->start_pos(0), 0.0001);
    EXPECT_NEAR(128.796, action->start_pos(1), 0.001);
    EXPECT_EQ(2, action->theta);
    EXPECT_NEAR(3.14159, action->speed, 0.00001);
    EXPECT_NEAR(2.44645, action->duration, 0.00001);

    action = m_actionspace.get_action(150);
    EXPECT_NEAR(106.783, action->start_pos(0), 0.001);
    EXPECT_NEAR(137.119, action->start_pos(1), 0.001);
    EXPECT_EQ(2, action->theta);
    EXPECT_NEAR(3.14159, action->speed, 0.00001);
    ...
    EXPECT_NEAR(143.155, action->start_pos(0), 0.001);
    EXPECT_NEAR(153.765, action->start_pos(1), 0.001);
    EXPECT_EQ(2, action->theta);
    EXPECT_NEAR(3.14159, action->speed, 0.00001);
    EXPECT_NEAR(0.815485, action->duration, 0.000001);

    action = m_actionspace.get_action(375);
    EXPECT_NEAR(161.341, action->start_pos(0), 0.001);
    EXPECT_NEAR(162.088, action->start_pos(1), 0.001);
    EXPECT_EQ(2, action->theta);
    EXPECT_NEAR(3.14159, action->speed, 0.00001);
    EXPECT_NEAR(0.271828, action->duration, 0.000001);

    action = m_actionspace.get_action(450);
    EXPECT_NEAR(28.7963, action->start_pos(0), 0.0001);
    EXPECT_NEAR(211.403, action->start_pos(1), 0.001);
    EXPECT_NEAR(0.429204, action->theta, 0.000001);
    EXPECT_NEAR(2.0944, action->speed, 0.0001);
    EXPECT_NEAR(2.71828, action->duration, 0.00001);

    action = m_actionspace.get_action(525);
    EXPECT_NEAR(37.1192, action->start_pos(0), 0.0001);
    EXPECT_NEAR(193.217, action->start_pos(1), 0.001);
    EXPECT_NEAR(0.429204, action->theta, 0.000001);
    EXPECT_NEAR(2.0944, action->speed, 0.0001);
    ...
    EXPECT_NEAR(53.7651, action->start_pos(0), 0.0001);
    EXPECT_NEAR(156.845, action->start_pos(1), 0.001);
    EXPECT_NEAR(0.429204, action->theta, 0.000001);
    EXPECT_NEAR(2.0944, action->speed, 0.0001);
    EXPECT_NEAR(1.08731, action->duration, 0.00001);

    action = m_actionspace.get_action(750);
    EXPECT_NEAR(62.088, action->start_pos(0), 0.0001);
    EXPECT_NEAR(138.659, action->start_pos(1), 0.001);
    EXPECT_NEAR(0.429204, action->theta, 0.000001);
    EXPECT_NEAR(2.0944, action->speed, 0.0001);
